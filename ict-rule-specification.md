...
## Prinsip kerja
- **Detection Engine** vs **Rule**: engine menyediakan primitif bersama (swing, struktur, waktu/sesi) yang dipakai berulang oleh banyak rule. Rule tidak boleh mengulang logika deteksi sendiri — hanya mengonsumsi output engine.
- **Primitive → Composite**: Primitive (Engine) menghasilkan fakta objektif (swing, structure_break, session level, FVG, Order Block, dll) — tidak mengambil keputusan trading. Composite (Rule) hanya menggabungkan output primitive untuk mengambil keputusan (VALID/INVALID/UNKNOWN). Tidak boleh ada Composite yang menghitung ulang atau menduplikasi algoritma primitive — kalau data yang dibutuhkan belum ada, yang diperbaiki adalah Primitive-nya, bukan menambah logika baru di Composite. Berlaku untuk seluruh dokumen — satu algoritma, satu sumber kebenaran.
