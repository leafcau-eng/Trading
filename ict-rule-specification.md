...
## DRT — Keputusan Arsitektur (v0.3.8)
...
```
Anchor (StructuralPoint EXTERNAL pertama yang unclaimed)
   |
Liquidity Sweep (fakta harga mentah -- wick menembus level; MEKANISME
   terkunci, tapi level MANA yang di-sweep relatif ke anchor: lihat UNKNOWN 2)
   |
StructureBreakEvent
   classification = EXTERNAL
   type = LAWAN dari anchor.type   (turunan langsung Q5+Q7+Q11, bukan
   keputusan baru -- pasangan valid cuma bisa high+low, anchor gak
   pernah ganti, jadi sisi lawan WAJIB tipe berlawanan)
   |
DRT Created (record immutable, range_high/range_low terkunci permanen)
```
